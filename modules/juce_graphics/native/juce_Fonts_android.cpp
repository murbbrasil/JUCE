...
namespace juce
{
// check Android API level at runtime
static bool isAndroidApiLevelAtLeast(int apiLevel)
{
    static int cachedApiLevel = []() {
        JNIEnv* env = getEnv();
    ...
    return cachedApiLevel >= apiLevel;
}
...
public:
    ...
    {
        // Compatibility implementation for API level 24+
        if (isAndroidApiLevelAtLeast(29))
        {
#if __ANDROID_API__ >= 29
            return matchWithAFontMatcher(text, language);
#else
            jassertfalse;
            return createLegacyFallback(text, language);
#endif
        }
        else
        {
...
private:
    // Legacy fallback implementation for API < 29
    Typeface::Ptr createLegacyFallback(const String& /*text*/, const String& /*language*/) const
    {
        if (getName().containsIgnoreCase("sans") || getName() == "Roboto")
            return from (FontOptions{}.withName ("Roboto"));
        else if (getName().containsIgnoreCase("serif"))
            return from (FontOptions{}.withName ("NotoSerif"));
        else if (getName().containsIgnoreCase("mono"))
            return from (FontOptions{}.withName ("DroidSansMono"));
        else if (getName().containsIgnoreCase("emoji"))
            return from (FontOptions{}.withName ("NotoColorEmoji"));

        return from (FontOptions{}.withName ("Roboto"));
    }

#if __ANDROID_API__ >= 29
    Typeface::Ptr matchWithAFontMatcher(const String& text, const String& language) const
    {
    ...
    static Typeface::Ptr findSystemTypefaceWithMatcher()
    {
        using AFontMatcherPtr = std::unique_ptr<AFontMatcher, FunctionPointerDestructor<AFontMatcher_destroy>>;
        using AFontPtr = std::unique_ptr<AFont, FunctionPointerDestructor<AFont_close>>;
    ...
    static Typeface::Ptr fromMatchedFont(AFont* matched)
    {
        if (matched == nullptr)
        {
            jassertfalse;
